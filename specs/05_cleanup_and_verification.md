...
- Попросить пользователя проверить и сообщить о результатах:
    - Проверить команды `/start` и `/help`
    - Проверить работу с Orchestrator
- После того как пользователь вернётся с ответами в том числе проверить сохранение сообщений в SQLite
...
- Проверить, что все документы в `docs/` обновлены согласно спекам 01-03. Например, docs/architecture/multi-agent-design.md мне кажется неактуальным
