...
*   **Iteration (Looping)**: Если Критик находит проблемы, Фасилитатор спрашивает пользователя, куда двигаться дальше:
    *   *Critic -> Dreamer*: Для поиска новых решений выявленных проблем.
    *   *Critic -> Phase 0*: Для корректировки постановки задачи, если поняли, что надо пересмотреть вводные (или по прямому запросу Пользователя/Критика).
    *   **ISOLATION RULE**: При возврате к Мечтателю, Фасилитатор **НЕ** показывает ему критику. Он формулирует новый "Вызов" или ограничение (например, "Придумай идеи дешевле"), чтобы не сбивать творческий настрой.
...
**Инструменты**:
Агенты (особенно Мечтатель и Реалист) активно используют **Web Search** для поиска аналогов, проверки фактов и расширения контекста. [в будущем]

Цикл завершается только тогда, когда **Пользователь** утверждает план (Blueprint), основываясь на отчете Критика.
            ...
            ELIF current_stage == "Critic":
                ...
                ELSE:
                    ...
                    IF feedback.requires_reimagining:
                        current_stage = "Phase 0" # Фундаментальные проблемы
                    ELSE:
                        current_stage = "Dreamer" # Продолжаем улучшать в рамках текущего запроса
...
*   **Интерактивность**:
    *   На этапе "Сбора запроса" Фасилитатор должен уметь задавать уточняющие вопросы пользователю
*   **Субагенты**:
